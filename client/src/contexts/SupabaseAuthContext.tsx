...
import { getSupabaseAuthRedirectUrl, getSupabasePasswordRecoveryRedirectUrl, getSupabaseProviderStatus, supabase, supabaseConfig, type SupabaseProviderStatus } from "@/lib/supabase";
import { markGoogleAuthPending } from "@/lib/googleAuth";
...
function friendlyError(message?: string) {
  ...
  if (/email address not authorized|not authorized/i.test(message)) return "Supabase's shared mailer only sends to authorized project-team addresses. Configure custom SMTP in Supabase for public member verification emails.";
  if (/rate limit|too many requests|over_email_send_rate_limit/i.test(message)) return "Too many verification emails were requested. Wait a few minutes, then use Resend confirmation once.";
...
export function SupabaseAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    async signUp(email, password, displayName) { const missing = configError(); if (missing) return { error: missing }; try { const { data, error } = await supabase.auth.signUp({ email, password, options: { emailRedirectTo: getSupabaseAuthRedirectUrl(window.location.origin), data: { display_name: displayName || undefined } } }); return error ? { error: friendlyError(error.message) } : { needsVerification: !data.session }; } catch (error) { return { error: friendlyError(error instanceof Error ? error.message : String(error)) }; } },
    async resendConfirmationEmail(email) { const missing = configError(); if (missing) return { error: missing }; try { const { error } = await supabase.auth.resend({ type: "signup", email, options: { emailRedirectTo: getSupabaseAuthRedirectUrl(window.location.origin) } }); return error ? { error: friendlyError(error.message) } : {}; } catch (error) { return { error: friendlyError(error instanceof Error ? error.message : String(error)) }; } },
    async signInWithGoogle() { const missing = configError(); if (missing) return { error: missing }; if (!providers.google) return { error: "Google sign-in is not enabled for this project yet. Use email/password or enable Google in Supabase Auth Providers." }; try { markGoogleAuthPending(); const { error } = await supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: getSupabaseAuthRedirectUrl(window.location.origin) } }); if (error) { window.sessionStorage.removeItem("promptforge_google_auth_pending"); return { error: friendlyError(error.message) }; } return {}; } catch (error) { window.sessionStorage.removeItem("promptforge_google_auth_pending"); return { error: friendlyError(error instanceof Error ? error.message : String(error)) }; } },
    async signInWithGitHub() { const missing = configError(); if (missing) return { error: missing }; if (!providers.github) return { error: "GitHub sign-in is not enabled for this project yet. Use email/password or enable GitHub in Supabase Auth Providers." }; try { const { error } = await supabase.auth.signInWithOAuth({ provider: "github", options: { redirectTo: getSupabaseAuthRedirectUrl(window.location.origin) } }); return error ? { error: friendlyError(error.message) } : {}; } catch (error) { return { error: friendlyError(error instanceof Error ? error.message : String(error)) }; } },
    async sendMagicLink(email) { const missing = configError(); if (missing) return { error: missing }; try { const { error } = await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: getSupabaseAuthRedirectUrl(window.location.origin) } }); return error ? { error: friendlyError(error.message) } : {}; } catch (error) { return { error: friendlyError(error instanceof Error ? error.message : String(error)) }; } },
    async sendPasswordReset(email) { const missing = configError(); if (missing) return { error: missing }; try { const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: getSupabasePasswordRecoveryRedirectUrl(window.location.origin) }); return error ? { error: friendlyError(error.message) } : {}; } catch (error) { return { error: friendlyError(error instanceof Error ? error.message : String(error)) }; } },
