...
import { getSupabaseAuthRedirectUrl, getSupabaseProviderStatus, supabase, supabaseConfig, type SupabaseProviderStatus } from "@/lib/supabase";
...
export function SupabaseAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    async sendPasswordReset(email) { const missing = configError(); if (missing) return { error: missing }; try { const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: getSupabaseAuthRedirectUrl(window.location.origin) }); return error ? { error: friendlyError(error.message) } : {}; } catch (error) { return { error: friendlyError(error instanceof Error ? error.message : String(error)) }; } },
