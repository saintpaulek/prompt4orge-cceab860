...
export function SupabaseAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    async signInWithGoogle() { const missing = configError(); if (missing) return { error: missing }; if (!providers.google) return { error: "Google sign-in is not enabled for this project yet. Use email/password or enable Google in Supabase Auth Providers." }; try { const { error } = await supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: getSupabaseAuthRedirectUrl(window.location.origin) } }); return error ? { error: friendlyError(error.message) } : {}; } catch (error) { return { error: friendlyError(error instanceof Error ? error.message : String(error)) }; } },
