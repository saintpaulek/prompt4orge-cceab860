...
export default function AdminUnlocks() {
  ...
  return <main className="admin-page access-bench">
    ...
    <section className="admin-panel inventory-panel"><div className="inventory-head"><div><div className="section-kicker">CODE INVENTORY <span>Audit trail</span></div><h2>Recent keys.</h2></div><button className="refresh-button" onClick={() => { void codes.refetch(); void audits.refetch(); }} disabled={codes.isFetching || audits.isFetching}><Loader2 size={14} className={codes.isFetching || audits.isFetching ? "spin" : ""}/> Refresh</button></div>{codes.isLoading ? <div className="page-loading"><Loader2 className="spin" size={18}/> Loading inventory…</div> : codes.data?.length ? <div className="inventory-table"><div className="inventory-row inventory-labels"><span>Code</span><span>Status</span><span>Created</span><span>Used by</span><span></span></div>{codes.data.map(row => <div className="inventory-row" key={row.id}><code>{row.code}</code><span className={row.isUsed ? "inventory-status used" : "inventory-status available"}>{row.isUsed ? "Redeemed" : "Available"}</span><span>{formatDate(row.createdAt)}</span><span>{row.usedBy ? `Member #${row.usedBy}` : "—"}</span><button aria-label={`Copy ${row.code}`} onClick={() => void copyCode(row.code)}><Copy size={14}/></button></div>)}</div> : <div className="empty-generated"><Users size={28}/><strong>No codes in inventory.</strong><span>Generate your first batch above.</span></div>}</section>
