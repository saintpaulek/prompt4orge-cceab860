...
import { AlertCircle, ArrowLeft, Check, Clipboard, Copy, KeyRound, Loader2, ShieldCheck, Sparkles, Users } from "lucide-react";
...
export default function AdminUnlocks() {
  ...
  const audits = trpc.admin.unlocks.audit.useQuery({ limit: 20 }, { enabled: isAdmin, retry: false });
  const redemptionAudits = trpc.admin.unlocks.redemptionAudit.useQuery({ limit: 100 }, { enabled: isAdmin, retry: false });
  ...
  return <main className="admin-page access-bench">
    ...
    <section className="admin-stats"><div className="admin-stat"><span>Total codes</span><strong>{stats.total}</strong><small>Last 100 generated</small></div><div className="admin-stat"><span>Available</span><strong>{stats.available}</strong><small>Ready to distribute</small></div><div className="admin-stat"><span>Redeemed</span><strong>{stats.used}</strong><small>Successfully tied to members</small></div><div className="admin-stat"><span>Failed attempts</span><strong>{stats.failed}</strong><small>Invalid code submissions</small></div></section>
    ...
    <section className="admin-panel inventory-panel"><div className="inventory-head"><div><div className="section-kicker">CODE INVENTORY <span>Audit trail</span></div><h2>Recent keys.</h2></div><button className="refresh-button" onClick={() => { void codes.refetch(); void audits.refetch(); void redemptionAudits.refetch(); }} disabled={codes.isFetching || audits.isFetching || redemptionAudits.isFetching}><Loader2 size={14} className={codes.isFetching || audits.isFetching || redemptionAudits.isFetching ? "spin" : ""}/> Refresh</button></div>{codes.isLoading ? <div className="page-loading"><Loader2 className="spin" size={18}/> Loading inventory…</div> : codes.data?.length ? <div className="inventory-table"><div className="inventory-row inventory-labels"><span>Code</span><span>Status</span><span>Created</span><span>Used by</span><span></span></div>{codes.data.map(row => <div className="inventory-row" key={row.id}><code>{row.code}</code><span className={row.isUsed ? "inventory-status used" : "inventory-status available"}>{row.isUsed ? "Redeemed" : "Available"}</span><span>{formatDate(row.createdAt)}</span><span>{row.usedBy ? `Member #${row.usedBy}` : "—"}</span><button aria-label={`Copy ${row.code}`} onClick={() => void copyCode(row.code)}><Copy size={14}/></button></div>)}</div> : <div className="empty-generated"><Users size={28}/><strong>No codes in inventory.</strong><span>Generate your first batch above.</span></div>}</section>
    <section className="admin-panel redemption-audit-panel"><div className="section-kicker">REDEMPTION ATTEMPTS <span>Account, result, and timestamp</span></div><h2>Recent redemption activity.</h2>{redemptionAudits.isLoading ? <div className="page-loading"><Loader2 className="spin" size={18}/> Loading redemption attempts…</div> : redemptionAudits.data?.length ? <div className="audit-list">{redemptionAudits.data.map(entry => <div className="audit-row" key={entry.id}><span>{entry.outcome === "SUCCESS" ? <Check size={14}/> : <AlertCircle size={14}/>} {entry.outcome === "SUCCESS" ? "Successful redemption" : entry.outcome === "ALREADY_USED" ? "Already redeemed" : entry.outcome === "RACE_LOST" ? "Concurrent redemption rejected" : "Failed attempt"}</span><strong>{entry.code}</strong><small>Member #{entry.userId} · {entry.userEmail || "Email unavailable"} · {formatDate(entry.createdAt)}</small></div>)}</div> : <p className="shelf-empty">No redemption attempts recorded yet.</p>}</section>
    <section className="admin-panel audit-panel"><div className="section-kicker">GENERATION LOG <span>Immutable batch events</span></div><h2>Recent batches.</h2>{audits.isLoading ? <div className="page-loading"><Loader2 className="spin" size={18}/> Loading audit log…</div> : audits.data?.length ? <div className="audit-list">{audits.data.map(entry => <div className="audit-row" key={entry.id}><span><Sparkles size={14}/> Batch generated</span><strong>{entry.generatedCount} code{entry.generatedCount === 1 ? "" : "s"}</strong><small>Admin #{entry.adminUserId} · {formatDate(entry.createdAt)}</small></div>)}</div> : <p className="shelf-empty">No generation events recorded yet.</p>}</section>
  </main>;
}
