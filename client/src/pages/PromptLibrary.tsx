...
import { ArrowRight, BookOpen, Check, ChevronDown, ChevronsUpDown, Clipboard, ExternalLink, LockKeyhole, Search, X } from "lucide-react";
...
export default function PromptLibrary() {
  ...
  const copyPrompt = async (item: LibraryPromptActionItem) => {
    ...
    try { await navigator.clipboard.writeText(item.promptBody); setCopiedId(item.id); window.setTimeout(() => setCopiedId(current => current === item.id ? null : current), 1600); trackProductEvent("prompt_copy", { prompt_id: item.id, category: item.category, access: item.access }); toast.success("Prompt copied", { description: "The full work order is ready to paste." }); } catch { toast.error("Copy unavailable", { description: "Your browser blocked clipboard access. Open the prompt to copy it manually." }); }
  };
  const useInBuilder = (item: LibraryPromptActionItem) => {
    ...
    trackProductEvent("prompt_use_in_builder", { prompt_id: item.id, category: item.category, access: item.access });
  ...
  return <main className="simple-page library-data-page">
    ...
    <div className="access-tabs" role="tablist" aria-label="Prompt access filter">{(["ALL", "FREE", "LOCKED"] as AccessFilter[]).map(item => <button key={item} id={`access-tab-${item.toLowerCase()}`} role="tab" aria-selected={access === item} tabIndex={access === item ? 0 : -1} className={access === item ? "active" : ""} onClick={() => setAccess(item)}>{item === "ALL" ? "All prompts" : item === "FREE" ? "Free prompts" : "Locked prompts"}</button>)}</div>
