...
export const appRouter = router({
  ...
  admin: router({
    unlocks: router({
      ...
      redemptionAudit: adminProcedure.input(z.object({ limit: z.number().int().min(1).max(200).default(100) }).optional()).query(({ input }) => listUnlockRedemptionAudits(input?.limit ?? 100)),
